...
export function Graph(props: { formula: string, calculation_vars: string; width: number; onCalculate(vars: { [key: string]: number }): void; data: {x: number, y: number}[] }) {
    ...
    return (
        <div style={{
            ...
            <div style={{ flexDirection: "column", display: 'flex', alignItems: props.width < 850 ? "flex-start" : 'center', width: '100%', justifyContent: 'space-between' }}>
                <div>
                    <ScatterChart
                        width={props.width < 800 ? 0.8 : 0.6}
                        height={props.width < 800 ? 0.8 : 0.6}
                        margin={{
